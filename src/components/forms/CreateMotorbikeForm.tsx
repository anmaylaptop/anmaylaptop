...
interface CreateMotorbikeFormProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
}

export function CreateMotorbikeForm({
  open,
  onOpenChange,
}: CreateMotorbikeFormProps) {
  const [isSubmitting, setIsSubmitting] = useState(false);
  const createMotorbike = useCreateMotorbike();
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      // Create motorbike
      await createMotorbike.mutateAsync({
        donor_id: donorId,
        student_id: null,
        brand: values.brand,
        model: values.model,
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Thêm xe máy mới vào kho. Bạn có thể chọn nhà hảo tâm có sẵn hoặc tạo mới.
          </DialogDescription>
        </DialogHeader>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            {/* Donor Selection */}
            <DonorSelector supportType="motorbike" />
